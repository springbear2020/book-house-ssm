...
3. 创建数据库和表并插入数据：
   - 登录 MySQL ，创建 `panda_book_house` 数据库
   - 将 `src/main/resources/panda_book_house.sql` 文件中的数据库表导入 panda_book_house 数据库中
4. 修改配置信息：
   - 修改 `src/main/resources/jdbc.properties` 中的数据库连接信息，设置你自己的用户名和密码 
   - 修改 `src/main/resources/email.properties` 中的邮箱连接信息，设置你自己的邮箱账号信息（[smtp 开启](https://blog.csdn.net/smilehappiness/article/details/108145215)）
5. 部署访问：在 IntelliJ IDEA 中部署 `Tomcat` 即可访问熊猫书屋首页
